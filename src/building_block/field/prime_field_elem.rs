use crate::building_block::{
    field::{prime_field::PrimeField, prime_field_elems::PrimeFieldElems},
    to_biguint::ToBigUint,
};
use bitvec::{prelude::Lsb0, view::BitView};
use num_bigint::{BigInt, BigUint, ToBigInt};
use num_traits::{One, ToPrimitive, Zero as NumTraitZero};
use std::{
    cmp::{Ord, Ordering, PartialOrd},
    fmt,
    ops::{Add, BitAnd, Deref, Div, Mul, Neg, Rem, ShrAssign, Sub},
    sync::Arc,
};
...
impl PrimeFieldElem {
    pub fn zero(&self) -> Self {
        // not using Zero trait since it requires self
        PrimeFieldElem {
            f: self.f.clone(),
            e: BigUint::from(0u8),
        }
...
mod tests {
    ...
    fn test_mul_large_number() {
        ...
        let rhs_e = BigUint::parse_bytes(
            b"63954422509139660694275478881573291931659433822585593108077818434106113196321",
            10,
        )
        .unwrap();
        let rhs = PrimeFieldElem::new(&f, &rhs_e);

        let exp_e = BigUint::parse_bytes(
            b"65344605666012213284100148944976995885360063020612010813911848313075706616617",
            10,
        )
        .unwrap();
    ...
    fn inv_secp256k1() -> Result<(), String> {
        ...
        let exp = BigUint::parse_bytes(
            b"52624297956533532283067125375510330718705195823487497799082320305224600546911",
            10,
        )
        .unwrap();
        let inv = a.safe_inv()?;
