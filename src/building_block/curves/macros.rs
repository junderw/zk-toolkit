...
macro_rules! impl_affine_add {
    ($point: ty) => {
        // cargo fmt is weird with this macro, skip it.
#[rustfmt::skip]
                ...
                impl Add<$rhs> for $target {
                    ...
                    fn add(self, rhs: $rhs) -> Self::Output {
                        if self.is_zero() && rhs.is_zero() {
                            ...
                            // adding p2 to zero is p2
                            rhs.clone()
                        } else if rhs.is_zero() {
                            // adding p1 to zero is p1
                            self.clone()
                        } else if self.x == rhs.x && self.y != rhs.y {
                            // if line through p1 and p2 is vertical line
                            self.zero()
                        } else if self.x == rhs.x && self.y == rhs.y {
                            // if adding the same point
