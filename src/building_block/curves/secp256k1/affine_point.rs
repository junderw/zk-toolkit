use crate::{
    building_block::{
        curves::{rational_point::RationalPoint, weierstrass_eq::WeierstrassEq},
        field::{prime_field::PrimeField, prime_field_elem::PrimeFieldElem},
        zero::Zero,
    },
    impl_affine_add, impl_scalar_mul_point,
};
...
impl AffinePoint {
    ...
    pub fn inv(&self) -> Self {
        match self {
            AffinePoint::AtInfinity => panic!("Cannot calculate the inverse of zero"),
            AffinePoint::Rational { x, y } => AffinePoint::new(x, y),
        }
    }
}
...
mod tests {
    ...
    fn add_same_point() {
        let g = &AffinePoint::g();
        let g2 = g + g;
        let exp_x = BigUint::parse_bytes(
            b"89565891926547004231252920425935692360644145829622209833684329913297188986597",
            10,
        )
        .unwrap();
        let exp_y = BigUint::parse_bytes(
            b"12158399299693830322967808612713398636155367887041628176798871954788371653930",
            10,
        )
        .unwrap();
        match g2 {
    ...
    fn get_g_multiples<'a>() -> Vec<AffinePoint> {
        let ps = vec![
            ...
            Xy {
                _n: "3",
                x: b"F9308A019258C31049344F85F89D5229B531C845836F99B08601F113BCE036F9",
                y: b"388F7B0F632DE8140FE337E62A37F3566500A99934C2231B6CB9FD7584B8E672",
            },
            Xy {
                _n: "4",
                x: b"E493DBF1C10D80F3581E4904930B1404CC6C13900EE0758474FA94ABE8C4CD13",
                y: b"51ED993EA0D455B75642E2098EA51448D967AE33BFBDFE40CFE97BDC47739922",
            },
            Xy {
                _n: "5",
                x: b"2F8BDE4D1A07209355B4A7250A5C5128E88B84BDDC619AB7CBA8D569B240EFE4",
                y: b"D8AC222636E5E3D6D4DBA9DDA6C9C426F788271BAB0D6840DCA87D3AA6AC62D6",
            },
            Xy {
                _n: "6",
                x: b"FFF97BD5755EEEA420453A14355235D382F6472F8568A18B2F057A1460297556",
                y: b"AE12777AACFBB620F3BE96017F45C560DE80F0F6518FE4A03C870C36B075F297",
            },
            Xy {
                _n: "7",
                x: b"5CBDF0646E5DB4EAA398F365F2EA7A0E3D419B7E0330E39CE92BDDEDCAC4F9BC",
                y: b"6AEBCA40BA255960A3178D6D861A54DBA813D0B813FDE7B5A5082628087264DA",
            },
            Xy {
                _n: "8",
                x: b"2F01E5E15CCA351DAFF3843FB70F3C2F0A1BDD05E5AF888A67784EF3E10A2A01",
                y: b"5C4DA8A741539949293D082A132D13B4C2E213D6BA5B7617B5DA2CB76CBDE904",
            },
            Xy {
                _n: "9",
                x: b"ACD484E2F0C7F65309AD178A9F559ABDE09796974C57E714C35F110DFC27CCBE",
                y: b"CC338921B0A7D9FD64380971763B61E9ADD888A4375F8E0F05CC262AC64F9C37",
            },
            Xy {
                _n: "10",
                x: b"A0434D9E47F3C86235477C7B1AE6AE5D3442D49B1943C2B752A68E2A47E247C7",
                y: b"893ABA425419BC27A3B6C7E693A24C696F794C2ED877A1593CBEE53B037368D7",
            },
        ];
        let g = &AffinePoint::g();
