use crate::building_block::{
    curves::{rational_point::RationalPoint, secp256k1::affine_point::AffinePoint},
    field::prime_field_elem::PrimeFieldElem,
    hasher::{hasher::Hasher, sha256::Sha256},
    zero::Zero,
};
    ...
    pub fn verify(&self, sig: &Signature, pub_key: &AffinePoint, message: &[u8]) -> bool {
        ...
        // check if r and s are in [1, n-1]
            ...
            match p3 {
                ...
                AffinePoint::Rational { x, y: _ } => sig.r.e == (x.e % n),
            }
        }
    }
}
...
mod tests {
    ...
    fn sign_verify_bad_pub_key() {
        ...
        match good_pub_key {
            ...
            AffinePoint::Rational { x, y: _ } => {
                let bad_pub_key = AffinePoint::new(&x, &x);
                match bad_pub_key {
