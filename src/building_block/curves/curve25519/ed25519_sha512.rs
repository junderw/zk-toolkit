...
use crate::building_block::{
    curves::curve25519::affine_point::{AffinePoint, Parity},
    field::prime_field_elem::PrimeFieldElem,
    hasher::{hasher::Hasher, sha512::Sha512},
};
...
impl Ed25519Sha512 {
    ...
    pub fn sign(&self, msg: &[u8], prv_key: &[u8; 32]) -> [u8; 64] {
        ...
        let r = BigUint::from_bytes_le(&self.H.get_digest(&prefix_msg));
        let r = r.rem(l);
        let R_pt = B * f.elem(&r);
        let R = self.encode_point(&R_pt);

        let R_A_msg = [&R, &A, msg].concat();
        let k = BigUint::from_bytes_le(&self.H.get_digest(&R_A_msg));
        let k = k.rem(l);
