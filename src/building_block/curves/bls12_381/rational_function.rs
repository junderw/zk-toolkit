use crate::building_block::{
    curves::bls12_381::{fq12::Fq12, g12_point::G12Point, g1_point::G1Point, g2_point::G2Point},
    ...
    ($point: ty, $func: ident) => {
        impl RationalFunction {
            pub fn $func(p: &$point, q: &$point) -> Self {
                match (p.into(), q.into()) {
                    (
                        G12Point::Rational { x: x1_12, y: y1_12 },
                        G12Point::Rational { x: x2_12, y: y2_12 },
                    ) => {
                        if p == q {
    ...
    ($point: ty, $func: ident) => {
        impl RationalFunction {
            ...
            pub fn $func(&self, q: &$point) -> Fq12 {
                ...
                match (q12, &self.line) {
                    (G12Point::Rational { x: X, y: _ }, Line::Vertical { x }) => X + -x,
