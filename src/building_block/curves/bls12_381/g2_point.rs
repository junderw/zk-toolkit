use crate::{
    building_block::{
        curves::{
            bls12_381::{
                fq1::Fq1, fq2::Fq2, params::Params as P, private_key::PrivateKey, reduce::Reduce,
            },
        ...
        impl Neg for $target {
            ...
            fn neg(self) -> Self::Output {
                match self {
                    ...
                    G2Point::Rational { x, y } => G2Point::new(&x, &y.neg()),
                }
            }
        }
    };
    ...
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (G2Point::Rational { x: x1, y: y1 }, G2Point::Rational { x: x2, y: y2 }) => {
                x1.u1 == x2.u1 && x1.u0 == x2.u0 && y1.u1 == y2.u1 && y1.u0 == y2.u0
            }
        }
    }
}
