...
use num_traits::Zero;
use std::{collections::HashMap, convert::From, ops::Mul};
...
impl SparseVec {
    ...
    pub fn set(&mut self, index: &impl ToBigUint, n: &impl ToBigUint) {
        ...
        if index >= &self.size {
            panic!(
                "Index {:?} is out of range. The size of vector is {:?}",
                index.e, self.size.e
            );
        }
        let n = self.f.elem(n);
...
impl Mul<&SparseVec> for &SparseVec {
    ...
    fn mul(self, rhs: &SparseVec) -> Self::Output {
        if self.size != rhs.size {
            panic!(
                "Expected size of rhs to be {:?}, but got {:?}",
                self.size.e, rhs.size.e
            );
        }
...
mod tests {
    ...
    fn test_slice() {
        ...
        let elems = vec![zero.clone(), one.clone(), two.clone(), three.clone()];
