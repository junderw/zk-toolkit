use crate::{
    building_block::{
        curves::bls12_381::g1_point::G1Point, curves::bls12_381::g2_point::G2Point,
...
impl CRS {
    ...
    pub fn new(f: &PrimeField, p: &PinocchioProver) -> Self {
        ...
        let g_v_alpha_v_k_mid: Vec<G1Point> = mid
            .iter()
            .map(|i| g1_v * alpha_v * &p.vi[*i].eval_at(s))
            .collect();
        let g_w_alpha_w_k_mid: Vec<G1Point> = mid
            .iter()
            .map(|i| g1_w * alpha_w * &p.wi[*i].eval_at(s))
            .collect();
        let g_y_alpha_y_k_mid: Vec<G1Point> = mid
            .iter()
            .map(|i| g_y * alpha_y * &p.yi[*i].eval_at(s))
            .collect();
