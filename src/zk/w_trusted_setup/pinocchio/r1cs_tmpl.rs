...
use crate::zk::w_trusted_setup::pinocchio::{constraint::Constraint, gate::Gate, term::Term};
...
impl R1CSTmpl {
    ...
    fn categorize_witness_terms(t: &Term, inputs: &mut Vec<Term>, mid: &mut Vec<Term>) {
        match t {
...
mod tests {
    ...
    fn test_r1cs_build_a_b_c_matrix() {
        ...
        assert_eq!(
            res[1],
            ("4 + t1".to_string(), "1".to_string(), "t2".to_string())
        );
        assert_eq!(
            res[2],
            ("t2".to_string(), "1".to_string(), "out".to_string())
        );
    }
