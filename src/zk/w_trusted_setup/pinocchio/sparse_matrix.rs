...
use crate::building_block::field::{prime_field::PrimeField, prime_field_elem::PrimeFieldElem};
use crate::building_block::to_biguint::ToBigUint;
use crate::zk::w_trusted_setup::pinocchio::{polynomial::Polynomial, sparse_vec::SparseVec};
use std::{collections::HashMap, convert::From, fmt, ops::Mul};
...
impl SparseMatrix {
    ...
    pub fn multiply_column(&self, col: &SparseVec) -> Self {
        if col.size != self.height {
            panic!(
                "column size is expected to be {:?}, but got {:?}",
...
impl Mul<&SparseMatrix> for &SparseMatrix {
    ...
    fn mul(self, rhs: &SparseMatrix) -> Self::Output {
        if self.width != rhs.height {
            panic!(
                "Can only multiply matrix with height {:?}, but the rhs height is {:?}",
