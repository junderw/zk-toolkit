use crate::building_block::{
    curves::secp256k1::{affine_point::AffinePoint, affine_points::AffinePoints},
    field::{prime_field_elem::PrimeFieldElem, prime_field_elems::PrimeFieldElems},
};
...
impl Bulletproofs {
    ...
    pub fn inner_product_argument(
        ...
        if n == &1 {
            ...
            Bulletproofs::inner_product_argument(&np, &ggp, &hhp, u, &Pp, &ap, &bp)
        }
    }
...
mod tests {
    ...
    fn test_field_elems_mul_field_elem() {
        ...
        let sL = PrimeFieldElems(vec![curve_group.elem(&2u8), curve_group.elem(&3u8)]);

        let exp = PrimeFieldElems(vec![curve_group.elem(&10u8), curve_group.elem(&15u8)]);
        let act = sL * x;
        assert!(act == exp);
    }
    ...
    fn test_mul_field_elem_above_order() {
        let curve_group = AffinePoint::curve_group();
        let g = &AffinePoint::g();
        let gg = &AffinePoints::new(&vec![g.clone(), g.clone()]);
